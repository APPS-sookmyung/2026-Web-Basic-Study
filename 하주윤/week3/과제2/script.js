...
async function fetchFacts(count = 1) {
  const lang = languageSelect.value; //  1. 사용자가 선택한 언어(language) 가져오기
  const url = `https://meowfacts.herokuapp.com/?count=${count}&lang=${lang}`; //  2. API 요청 URL 생성!!!! 힌트: const url='$_?_= $_&_=$_'

  try {
    showLoading(); //  3. 로딩 상태 표시
    // ✅ 4. API 요청을 보내고 응답을 기다림
    const response = await fetch(url); // → fetch()를 사용하여 네트워크 요청을 보낸다.
    ...
    const data = await response.json(); // 5. JSON 데이터 변환

    return data.data; // 6. 데이터 return(반환환) (data.data 배열)
  } catch (err) {
    console.error("Error:", err.message);
    alert("데이터를 가져오는 중 오류가 발생했습니다."); //  7. 에러 발생 시 사용자에게 알림
  } finally {
    hideLoading(); // 8. 요청이 끝나면 로딩 상태 숨기기
  }
}
...
getFactBtn.addEventListener("click", async () => {
  const facts = await fetchFacts(1); //  1. API에서 하나의 랜덤한 고양이 사실 가져오기
...
getMultipleBtn.addEventListener("click", async () => {
  const count = parseInt(factCount.value) || 1; //  1. 입력된 개수 가져오기 (기본값 1) 힌트: parseInt 사용
  if (isNaN(count) || count < 1 || count > 5) {
    alert("Please enter a number between 1 and 5");
    return;
  } //  2. 유효한 개수(1~5)인지 확인 -> 유효하지 않으면 에러 메시지 표시 "Please enter a number between 1 and 5"
  const facts = await fetchFacts(count); //  3. API 호출하여 여러 개의 팩트 가져오기 (힌트: fetchFacts(count) 사용)
  if (facts) {
    displayFacts(facts);
